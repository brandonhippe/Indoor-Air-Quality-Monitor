// Indoor Air Quality V3, created to use Sensirion SCD41 instead of SGP30
...
#define ANEM_SLEEP_PIN 7   // Set Anemometer sleep pin to 5 to enable, 0 to disable
#define PM_SLEEP_PIN 6    // Set PM sleep pin to 6 to enable, 0 to disable
#define CO2_SLEEP_PIN 0   // Set CO2 sleep pin to 7 to enable, 0 to disable
...
// Periods for ultrasonic anemometer haven't been tested with SCD41.
const uint64_t pm_periods[3][3] =   {{6600000, 33600000, 6300000},
                                     {2340000,  3060000, 2880000},
                                     { 720000,  2100000, 1320000}};
                                  
const uint64_t co2_periods[3][3] =  {{ 480000, 14100000, 3060000},
                                     { 180000,  2340000, 1500000},
                                     {  30000,   900000,  720000}};
...
void setup() {
  ...
  for (int i = 1; i <= 40; i++) {
    digitalWrite(i, (i != 8 && i != 32 && i != 7 && i != 5));
    pinMode(i, OUTPUT);
  }
  ...
  if (CO2_SLEEP_PIN) digitalWrite(CO2_SLEEP_PIN, LOW); 
  if (ANEM_SLEEP_PIN) digitalWrite(ANEM_SLEEP_PIN, LOW); 
  if (!sensed) {
  ...
  sensed = co2.begin(debug, CO2_MODE, CO2_SLEEP_PIN);
  if (ANEM_SLEEP_PIN) digitalWrite(ANEM_SLEEP_PIN, LOW); 
  if (!sensed) {
...
//    while (!powerTest);
  ...
  if (powerTest) {
    pm.setPeriod(0xFFFFFFFF);
    co2.setPeriod(0xFFFFFFFF);
    anem.setPeriod(0xFFFFFFFF);

    // ADD SENSOR TO POWER TEST HERE!
//    co2.setPeriod(60000);
  } else {
    ...
    if (anem_present) {
      ...
      if (BATTERY_LIFE == 2) co2.MODE = LP_MODE;
  ...
  if (!digitalRead(LB) && !alertSent) {
    ...
    switch (nextDevice) {
      ...
      case PM:
        ...
        if (anem_present && ANEM_SLEEP_PIN != 0 && ANEM_OFFSET == 0 && !anem.measurement_started) anem.startNextFunc(time_ms + startTime - millis());
        if (CO2_SLEEP_PIN != 0 && CO2_MODE == PC_MODE) co2.scd41_wakeup_transistor();
        pm.startNextFunc(time_ms + startTime - millis());
        if (pm.measurement_ready && CO2_SLEEP_PIN != 0 && CO2_MODE == PC_MODE) co2.scd41_sleep_transistor();
      ...
      case ANEM:
        ...
        if (anem.measurement_ready && CO2_SLEEP_PIN != 0 && CO2_MODE == PC_MODE) co2.scd41_sleep_transistor();
